...
export (int) var jump_speed = -600   #speed of the jump
export (int) var gravity = 1500  #gravity
var run_speed = 500 
...
func get_input():
    ...
    if velocity.y < -100 && !jump_pressed: # here is the deal: if the jump button is not keep pressed anymore, the y velocity is set to 0 and the player don't go up anymore
        velocity.y = -100
